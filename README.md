...
- **Agent-level file access (opt-in).** With the toggle on, a model can write a whole real, runnable multi-file project in one response instead of dumping code blocks to copy by hand — same response-convention approach as the document panel, so it needs no native tool-calling support from any provider. Defaults to a "Token Thrift Projects" folder under Documents (one subfolder per project), or point it at a specific folder of your own — an existing repo you want it working inside, say — and it writes straight there instead. Either way, it also gets a live snapshot of that folder's current contents (file listing plus small text files' contents) folded into its context every turn, so it can extend an existing project accurately instead of only ever scaffolding from nothing. A follow-up request to the same project resumes into its existing folder instead of duplicating it.
- **A session-scoped file library** — every uploaded or generated file is saved to disk and shown in a grid, sized and colored by file size, one click from opening in whatever your OS treats as the default handler for that file type. A model-written project shows up as a triangular link tile instead, pointing at its real folder outside the library's own storage — drag a replacement file or folder onto it to relink if the target ever moves or gets deleted.
