...
- **Agent-level file access (opt-in).** With the toggle on, a model can write a whole real, runnable multi-file project to your Documents folder in one response instead of dumping code blocks to copy by hand — same response-convention approach as the document panel, so it needs no native tool-calling support from any provider. A follow-up request to the same project resumes into its existing folder instead of duplicating it.
